...
def pdhg_initialize_variables(m, n, device, Z0=None, Y0=None, dtype=torch.float32): 
    if Z0 is not None:
        Z = Z0 
    else:
        Z = torch.zeros((2 * m, n), device=device, dtype=dtype)
    Z_bar = Z.clone() 
    if Y0 is not None:
        Y = Y0
    else:
...
def pdhg_kq_attn_layer(
    ...
    n = int(n_sq ** 0.5)
    m = mn2 // (2 * n)
    Z, Z_bar, Y = pdhg_initialize_variables(m, n, device=A_linop.device, Z0=Z0, Y0=Y0, dtype=Grad.dtype) 
    if mu > 0:
    ...
    if diag_scaling:
        R, Gamma1, Gamma2 = pdhg_diagonal_scaling(A=A_linop.A2, B=A_linop.A1, eta=0.99)
        ...
        Gamma = torch.ones((2 * m, 1), device=A_linop.device, dtype=A_linop.dtype) 
...
@torch.no_grad()
def pdhg_kq_attn_layer_matrices(
    prox_h_conj,
    A1: torch.Tensor,
    A2: torch.Tensor,
    G1: torch.Tensor,
    ...
    m, n = A1.shape
    Z, Z_bar, Y = pdhg_initialize_variables(m, n, A1.device, Z0=Z0, Y0=Y0, dtype=G1.dtype)
    if mu > 0:
