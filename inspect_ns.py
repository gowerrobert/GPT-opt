...
ns_accel_A_pinv, ns_accel_diagnostics = accelerated_ns_pinv(A, l*power_norm, 1.1*power_norm, 80, psd=False, early_stop_eps=eigvals.min(), dtype=dtype, diagnostics=True)
...
_, add_eps_diagnostics = accelerated_ns_pinv(A, l*power_norm, 1.1*power_norm, 80, psd=True, add_eps=1e-3, early_stop_eps=eigvals.abs().min(), dtype=dtype, diagnostics=True)
