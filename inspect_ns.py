...
if True:  # clip negative eigenvalues
    A = (eigvecs @ torch.maximum(torch.diag(eigvals), torch.tensor(1e-6)) @ eigvecs.T).to(torch.float32)

dtype = torch.float64
l = 1e-6

power_norm = power_method(A, psd=True, use_bf16=False)
eps = power_norm * l
print("power_norm =", power_norm.item(), "\nfrobenius norm =", A.norm().item(), "\ntrue norm =", eigvals.abs().max().item())
print(f"eps={eps}")
...
ns_accel_A_pinv, ns_accel_diagnostics = accelerated_ns_pinv(A, l*power_norm, 1.1*power_norm, 80, psd=True, dtype=dtype, diagnostics=True)
